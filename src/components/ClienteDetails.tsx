...
import { ArrowLeft, User, Mail, Phone, MapPin, Calendar, CreditCard, Briefcase, Heart } from 'lucide-react';
...
const ClienteDetails: React.FC<ClienteDetailsProps> = ({ cliente, contas, agencia, onBack }) => {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
          <div>
            ...
            {cliente.nomeSocial && (
              <p className="text-gray-600 mb-3">Nome social: {cliente.nomeSocial}</p>
            )}
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex flex-col sm:flex-row gap-4">
              <div>
                ...
                <p className="font-semibold text-green-700">{formatCurrency(cliente.rendaAnual)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Patrimônio</p>
                <p className="font-semibold text-green-700">{formatCurrency(cliente.patrimonio)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-gray-800">{formatDate(cliente.dataNascimento)}</p>
            </div>
          </div>
      ...
      <h3 className="text-xl font-semibold text-gray-800 mb-3">Informações da Agência</h3>
      <div className="mb-6">
      ...
      {contas.length === 0 ? (
        <div className="bg-gray-100 p-4 rounded-md">
          <p className="text-gray-500 text-center">Este cliente não possui contas bancárias</p>
        </div>
      ) : (
